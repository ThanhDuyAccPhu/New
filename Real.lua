...
if not WindUI then
    ...
    -- K1NG TECH TAB
    -- ============================================
    -- K1ng Tech config (from k1ngMod.lua, integrated into WindUI)
    ...
    local K1NG_UPPERCUT = {["rbxassetid://10503381238"] = true, ["rbxassetid://13379003796"] = true}
    local K1NG_DETECT_ANIMS = {["rbxassetid://10479335397"] = true, ["rbxassetid://13380255751"] = true}
    ...
    local function K1ngAutoPressQ()
        ...
        local comm = char:FindFirstChild("Communicate")
        if comm then
            pcall(function() comm:FireServer({[1] = {Dash = Enum.KeyCode.W, Key = Enum.KeyCode.Q, Goal = "KeyPress"}}) end)
        end
        pcall(function()
    ...
    local function K1ngForceJump()
        ...
        root.AssemblyLinearVelocity = Vector3.new(root.AssemblyLinearVelocity.X, K1ngTechConfig.VELO_POWER, root.AssemblyLinearVelocity.Z)
    end
    ...
    local function K1ngCustomFlip()
        ...
        if hum and root then
            ...
            Camera.CFrame = CFrame.new(flippedCFrame.Position - flippedCFrame.LookVector * (Camera.CFrame.Position - flippedCFrame.Position).Magnitude + Vector3.new(0, 2), flippedCFrame.Position)
        end
    end
    ...
    local function K1ngSetupCharacter(char)
        if K1ngTechConfig.AnimConnection then
            K1ngTechConfig.AnimConnection:Disconnect()
            K1ngTechConfig.AnimConnection = nil
        end
        local hum = char:WaitForChild("Humanoid")
        K1ngTechConfig.AnimConnection = hum.AnimationPlayed:Connect(function(track)
            ...
            if K1NG_UPPERCUT[animId] and not K1ngTechConfig.onCooldown then
                ...
                task.delay(K1ngTechConfig.DETECT_DELAY, function()
                    if K1ngTechConfig.kingBusy or not K1ngTechConfig.Enabled or K1ngTechConfig.onCooldown or tick() - triggerTime > 0.4 then return end
    ...
    local K1ngTechTab = Window:Tab({
        Title = "K1ng Tech",
        Icon = "zap",
        IconColor = Color3.fromRGB(168, 85, 247),
    })
    ...
    K1ngTechTab:Paragraph({
        Title = "Thông Tin",
        Content = "K1ng Tech - Auto tech khi bị uppercut. Tự động nhảy, flip và dash khi phát hiện animation.",
    })
    ...
    K1ngTechTab:Toggle({
        ...
        Callback = function(state)
            ...
            if state then
                local char = LocalPlayer.Character
                if char then K1ngSetupCharacter(char) end
                if not K1ngTechConfig.CharConnection then
                    K1ngTechConfig.CharConnection = LocalPlayer.CharacterAdded:Connect(function(newChar)
                        K1ngTechConfig.Enabled = false
                        K1ngTechConfig.onCooldown = false
                        K1ngTechConfig.kingBusy = false
                        if K1ngTechConfig.Enabled then
                            task.wait(0.5)
                            K1ngSetupCharacter(newChar)
                        end
                    end)
                end
                WindUI:Notify({
                    Title = "K1ng Tech",
                    Content = "Đã bật K1ng Tech!",
                    Icon = "zap",
                    Duration = 2,
                })
            else
    ...
    K1ngTechTab:Slider({
        ...
        Step = 0.01,
        Callback = function(value)
            K1ngTechConfig.DETECT_DELAY = value
        end
    })
    ...
    K1ngTechTab:Slider({
        Title = "Velocity Power (nhảy)",
        Value = {Min = 20, Max = 120, Default = 60},
        Step = 5,
        Callback = function(value)
            K1ngTechConfig.VELO_POWER = value
        end
    })
    ...
    local MUSIC_LIST = {
        {Name = "Call Of Silence", Id = "rbxassetid://123878926068911"},
    }
    local SelectedMusicIndex = 1

    local function K_CreateSound()
        if MusicConfig.Sound then
            pcall(function() MusicConfig.Sound:Stop() MusicConfig.Sound:Destroy() end)
            MusicConfig.Sound = nil
        end
        local sound = Instance.new("Sound")
        sound.Name = "YumeXHubMusic"
        sound.SoundId = MUSIC_LIST[SelectedMusicIndex].Id
        sound.Volume = MusicConfig.Volume
        sound.Looped = true
        sound.RollOffMaxDistance = 10000
        sound.Parent = workspace
        MusicConfig.Sound = sound
        return sound
    end

    local function K_PlayMusic()
        local s = K_CreateSound()
        s:Play()
        MusicConfig.IsPlaying = true
    end

    local function K_StopMusic()
        if MusicConfig.Sound then
            pcall(function() MusicConfig.Sound:Stop() end)
        end
    ...
    MusicTab:Paragraph({
        ...
        Content = "ID: 123878926068911 | Nhạc OST anime nổi tiếng.",
    })
    ...
    MusicTab:Button({
        Title = "▶  Bật Nhạc",
        Description = "Phát: " .. MUSIC_LIST[SelectedMusicIndex].Name,
        Icon = "play",
        Color = PINK,
        Callback = function()
            K_PlayMusic()
            WindUI:Notify({
                Title = "Music",
                Content = "Đang phát: " .. MUSIC_LIST[SelectedMusicIndex].Name,
                Icon = "music",
                Duration = 3,
            })
        end
    ...
    MusicTab:Slider({
        ...
        Callback = function(value)
            ...
            if MusicConfig.Sound then
                MusicConfig.Sound.Volume = value
            end
        end
    })

    MusicTab:Button({
        Title = "🔊 Tăng Âm Lượng (+0.1)",
        Icon = "volume-2",
        Color = PINK,
        Callback = function()
            MusicConfig.Volume = math.min(1, MusicConfig.Volume + 0.1)
            MusicConfig.Volume = math.floor(MusicConfig.Volume * 100 + 0.5) / 100
            if MusicConfig.Sound then
                MusicConfig.Sound.Volume = MusicConfig.Volume
            end
            WindUI:Notify({
                Title = "Âm Lượng",
                Content = "Âm lượng: " .. tostring(math.floor(MusicConfig.Volume * 100)) .. "%",
                Icon = "volume-2",
                Duration = 2,
            })
        end

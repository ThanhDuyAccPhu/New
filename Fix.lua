...
-- ============================================
-- KITTY TECH CONFIG & LOGIC (UPDATED FROM FILE 2)
-- ============================================
...
local function KittyTechOnAnimationPlayed(anim)
    if KittyTechConfig.Enabled and not KittyTechConfig.Busy then
        local animId = tostring(anim.Animation.AnimationId)
        if animId:find("10503381238") then            task.spawn(KittyTechExecute)
        end
    end
end
...
InfoTab:Button({
    ...
    Callback = function()
        ...
        clipboard("https://discord.gg/W8vpy5zH5")
        WindUI:Notify({
            Title = "Copied!",
...
InfoTab:Button({
    ...
    Callback = function()
        ...
        clipboard("https://www.tiktok.com/@usertd1609?_r=1&_t=ZS-96c7SgM2dj3")
        WindUI:Notify({
            Title = "Copied!",
...
-- ============================================
-- KITTY TECH TAB (UPDATED WITH ALL SETTINGS FROM FILE 2)
-- ============================================
...
if LocalPlayer.Character then
    ...
    if SupaLegitConfig.Enabled then ConnectSupaLegit() end

    ...
    InstantLethalConfig.Connection = hum.AnimationPlayed:Connect(function(anim)
        if InstantLethalConfig.Enabled and anim.Animation and anim.Animation.AnimationId == InstantLethalConfig.AnimationId then
            ...
            if char and char:FindFirstChild("Communicate") then
                pcall(function() char.Communicate:FireServer(unpack({{Dash = Enum.KeyCode.W, Key = Enum.KeyCode.Q, Goal = "KeyPress"}})) end)
            end
...
local function AutoBlockAction(distance, delayTime)
    ...
    pcall(function() comm:FireServer(unpack(args2)) end)

    if AutoBlockConfig.M1AfterBlock and distance <= 15 then
        local args3 = {{Goal="LeftClick", Mobile=true}}
        pcall(function() comm:FireServer(unpack(args3)) end)
        task.wait(0.3)
        local args4 = {{Goal="LeftClickRelease", Mobile=true}}
        pcall(function() comm:FireServer(unpack(args4)) end)
    end
end
...
local function AutoBlockSpamReleases()
    ...
    for i = 1, 5 do
        pcall(function() comm:FireServer({{Goal="KeyRelease",Key=Enum.KeyCode.F}}) end)
        pcall(function() comm:FireServer({{Goal="LeftClickRelease",Mobile=true}}) end)
        task.wait(0.01)
    end
end

...
-- ============================================
-- WINDUI WINDOW SETUP (CẬP NHẬT THEO YÊU CẦU)
-- ============================================
if not WindUI then
    -- Create simple GUI if WindUI fails (giữ nguyên)
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "YumeXHub_Fallback"
    ...
    local Window = WindUI:CreateWindow({
        ...
        -- Các thuộc tính mới theo yêu cầu
        Background = "rbxassetid://71274108065842",
    ...
    -- ============================================
    -- K1NG TECH TAB (MỚI) - ĐẶT TRƯỚC LOOP DASH
    -- ============================================
    ...
    KingTechTab:Section({
        Title = "K1ng Tech - Auto Attack Enhancer",
        Icon = "zap",
    })
    
    KingTechTab:Paragraph({
        Title = "Thông Tin",
        Content = "Kích hoạt K1ng Tech để tự động phản ứng khi đối thủ dùng chiêu (uppercut, dash). Có thể tùy chỉnh độ trễ, lực nhảy, v.v.",
    })
    
    -- Cấu hình cho K1ng Tech (được chuyển từ file k1ngMod.lua)
    local KingTechSettings = {
        Enabled = false,
        DetectDelay = 0.25,
        StartDelay = 0,
        WaitTime = 0.15,
        ...
        Connection = nil,
        CharAddedConn = nil,
        ButtonGui = nil,      -- GUI button của K1ng Tech
        ...
        Busy = false,
        CurrentCooldownGui = nil,
    }
    
    -- Lấy DashRemote (theo cách của file 2)
    local DashRemote = nil
    pcall(function()
        DashRemote = ReplicatedStorage:WaitForChild("Resources"):WaitForChild("Brother")["#Friend"].Communicate
    end)
    if not DashRemote then
        -- fallback: dùng communicate từ character
        DashRemote = function(...) 
            local char = LocalPlayer.Character
            if char and char:FindFirstChild("Communicate") then
    ...
    local function KingTech_GetRoot(char) return char and char:FindFirstChild("HumanoidRootPart") end
    local function KingTech_GetClosestEnemy(maxDist)
        local char = LocalPlayer.Character if not char then return nil end
        local myRoot = KingTech_GetRoot(char) if not myRoot then return nil end
        local live = Workspace:FindFirstChild("Live") if not live then return nil end
        local best, shortest = nil, maxDist or 14
        for _, model in ipairs(live:GetChildren()) do
            if model ~= char then
                local hum = model:FindFirstChildOfClass("Humanoid")
                local root = KingTech_GetRoot(model)
    ...
    local function KingTech_AutoPressQ()
        if DashRemote then
            pcall(function() DashRemote:FireServer({[1] = {Dash = Enum.KeyCode.W, Key = Enum.KeyCode.Q, Goal = "KeyPress"}}) end)
        end
        pcall(function()
    ...
    local function KingTech_ForceJump()
        local char = LocalPlayer.Character if not char then return end
        local root = KingTech_GetRoot(char) if not root then return end
        root.AssemblyLinearVelocity = Vector3.new(root.AssemblyLinearVelocity.X, KingTechSettings.VelocityPower, root.AssemblyLinearVelocity.Z)
    end
    
    local function KingTech_CustomFlip()
        local char = LocalPlayer.Character if not char or not Camera then return end
        local root, hum = KingTech_GetRoot(char), char:FindFirstChildOfClass("Humanoid")
        if hum and root then
            hum.AutoRotate = false
            local flippedCFrame = root.CFrame * CFrame.Angles(0, math.rad(180), 0)
            root.CFrame = flippedCFrame
            local camPos = Camera.CFrame.Position
            local dist = (camPos - flippedCFrame.Position).Magnitude
            Camera.CFrame = CFrame.new(flippedCFrame.Position - flippedCFrame.LookVector * dist + Vector3.new(0, 2), flippedCFrame.Position)
        end
    end
    
    local function KingTech_StartCooldown(time)
        local char = LocalPlayer.Character if not char then return end
        local root = KingTech_GetRoot(char) if not root then return end
        if KingTechSettings.CurrentCooldownGui then KingTechSettings.CurrentCooldownGui:Destroy() end
        ...
        gradient.Parent = bar
        KingTechSettings.CurrentCooldownGui = bill
        TweenService:Create(bill, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(4, 0, 0.5, 0)}):Play()
        local start = tick()
        while tick() - start < time do
            bar.Size = UDim2.new(1 - ((tick() - start) / time), 0, 1, 0)
            RunService.RenderStepped:Wait()
        end
        local tween = TweenService:Create(bill, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Size = UDim2.new(0, 0, 0, 0)})
        tween:Play()
        tween.Completed:Wait()
        if KingTechSettings.CurrentCooldownGui then
            KingTechSettings.CurrentCooldownGui:Destroy()
            KingTechSettings.CurrentCooldownGui = nil
        end
    end
    
    local UPPERCUT_ANIMS = {["rbxassetid://10503381238"] = true, ["rbxassetid://13379003796"] = true}
    local DETECT_ANIMS = {["rbxassetid://10479335397"] = true, ["rbxassetid://13380255751"] = true}
    
    local function KingTech_OnAnimation(track)
        if not KingTechSettings.Enabled or not track.Animation then return end
        local animId = track.Animation.AnimationId
        if DETECT_ANIMS[animId] then
            if KingTechSettings.CooldownActive then return end
            KingTechSettings.CooldownActive = true
            task.delay(0.8, function() KingTech_StartCooldown(KingTechSettings.CooldownTime) end)
            task.delay(KingTechSettings.CooldownTime, function() KingTechSettings.CooldownActive = false end)
        end
        if UPPERCUT_ANIMS[animId] and not KingTechSettings.CooldownActive then
            local triggerTime = tick()
            task.delay(KingTechSettings.DetectDelay, function()
                if KingTechSettings.Busy or not KingTechSettings.Enabled or KingTechSettings.CooldownActive or tick() - triggerTime > 0.4 then return end
                local enemy = KingTech_GetClosestEnemy(14)
                if not enemy then return end
                KingTechSettings.Busy = true
                task.delay(KingTechSettings.StartDelay, function()
                    KingTech_AutoPressQ()
                    KingTech_ForceJump()
                    task.wait(KingTechSettings.WaitTime)
                    KingTech_CustomFlip()
                    task.wait(0.08)
                    KingTech_CustomFlip()
                    local char = LocalPlayer.Character
                    local hrp = char and KingTech_GetRoot(char)
                    if hrp then
                        TweenService:Create(hrp, TweenInfo.new(0.12, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {CFrame = hrp.CFrame + (hrp.CFrame.LookVector * 5)}):Play()
                    end
                end)
                task.delay(0.8, function() KingTechSettings.Busy = false end)
            end)
        end
    end
    
    local function KingTech_SetupCharacter(char)
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            if KingTechSettings.Connection then KingTechSettings.Connection:Disconnect() end
            KingTechSettings.Connection = hum.AnimationPlayed:Connect(KingTech_OnAnimation)
        end
    end
    
    -- Tạo GUI button riêng cho K1ng Tech (giống file 2)
    local function KingTech_CreateButtonGUI()
        if KingTechSettings.ButtonGui then
            KingTechSettings.ButtonGui:Destroy()
            KingTechSettings.ButtonGui = nil
        end
        local gui = Instance.new("ScreenGui")
        gui.Name = "K1ngTechGui"
        ...
        btn.Text = "<i><font face='Arcade'><font color='#a855f7'>K</font><font color='#b066ff'>1</font><font color='#b878ff'>n</font><font color='#c089ff'>g</font> <font color='#a855f7'>T</font><font color='#b066ff'>e</font><font color='#b878ff'>c</font><font color='#c089ff'>h</font></font></i>"
        ...
        local function startGradient()
            if KingTechSettings.GradientRunning then return end
            KingTechSettings.GradientRunning = true
            KingTechSettings.GradientThread = task.spawn(function()
                while KingTechSettings.GradientRunning do
                    local tween = TweenService:Create(bgGradient, TweenInfo.new(6, Enum.EasingStyle.Linear), {Rotation = 360})
                    tween:Play()
                    tween.Completed:Wait()
                    bgGradient.Rotation = 0
                end
        ...
        local function stopGradient()
            KingTechSettings.GradientRunning = false
            if KingTechSettings.GradientThread then
                coroutine.close(KingTechSettings.GradientThread)
                KingTechSettings.GradientThread = nil
            end
            bgGradient.Rotation = 0
        end
        
        btn.MouseButton1Click:Connect(function()
            KingTechSettings.Enabled = not KingTechSettings.Enabled
            if KingTechSettings.Enabled then
                startGradient()
                btn.Text = "<i><font face='Arcade'><font color='#a855f7'>K</font><font color='#b066ff'>1</font><font color='#b878ff'>n</font><font color='#c089ff'>g</font> <font color='#a855f7'>T</font><font color='#b066ff'>e</font><font color='#b878ff'>c</font><font color='#c089ff'>h</font><font color='#e9d5ff'> ✓</font></font></i>"
                btn.BackgroundColor3 = Color3.fromRGB(40,40,40)
                -- Kích hoạt script: setup character hiện tại
                if LocalPlayer.Character then
                    KingTech_SetupCharacter(LocalPlayer.Character)
                end
                if not KingTechSettings.CharAddedConn then
                    KingTechSettings.CharAddedConn = LocalPlayer.CharacterAdded:Connect(function(char)
                        if KingTechSettings.Enabled then
                            task.wait(0.5)
                            KingTech_SetupCharacter(char)
                        end
                    end)
                end
            else
                stopGradient()
                btn.Text = "<i><font face='Arcade'><font color='#a855f7'>K</font><font color='#b066ff'>1</font><font color='#b878ff'>n</font><font color='#c089ff'>g</font> <font color='#a855f7'>T</font><font color='#b066ff'>e</font><font color='#b878ff'>c</font><font color='#c089ff'>h</font><font color='#e0ceff'> X</font></font></i>"
                btn.BackgroundColor3 = Color3.fromRGB(40,40,40)
                -- Hủy kết nối
                if KingTechSettings.Connection then
                    KingTechSettings.Connection:Disconnect()
                    KingTechSettings.Connection = nil
                end
                if KingTechSettings.CharAddedConn then
                    KingTechSettings.CharAddedConn:Disconnect()
                    KingTechSettings.CharAddedConn = nil
                end
                KingTechSettings.Busy = false
                KingTechSettings.CooldownActive = false
                if KingTechSettings.CurrentCooldownGui then
                    KingTechSettings.CurrentCooldownGui:Destroy()
                    KingTechSettings.CurrentCooldownGui = nil
                end
        ...
        KingTechSettings.ButtonGui = gui
        KingTechSettings.ButtonGradient = bgGradient
        -- Mặc định khi tạo GUI thì chưa bật, nếu toggle trong WindUI đang bật thì đồng bộ
        if KingTechSettings.Enabled then
            btn.MouseButton1Click:Fire() -- tự bật nếu đã được bật từ trước (hiếm)
        end
    end
    
    -- Toggle trong WindUI để điều khiển K1ng Tech (đồng bộ với button GUI)
    local kingToggleState = false
    KingTechTab:Toggle({
        ...
        Color = PINK,
        Callback = function(state)
            kingToggleState = state
            if state then
                if not KingTechSettings.ButtonGui then
                    KingTech_CreateButtonGUI()
                end
                -- Nếu button GUI chưa ở trạng thái bật, simulate click
                if KingTechSettings.ButtonGui and KingTechSettings.ButtonGui:FindFirstChildOfClass("TextButton") then
                    local btn = KingTechSettings.ButtonGui:FindFirstChildOfClass("TextButton")
                    if btn and not KingTechSettings.Enabled then
                ...
                if KingTechSettings.ButtonGui then
                    local btn = KingTechSettings.ButtonGui:FindFirstChildOfClass("TextButton")
                    if btn and KingTechSettings.Enabled then
                        btn.MouseButton1Click:Fire()
                    end
                    KingTechSettings.ButtonGui:Destroy()
                    KingTechSettings.ButtonGui = nil
                ...
                KingTechSettings.Enabled = false
                KingTechSettings.Busy = false
                KingTechSettings.CooldownActive = false
                if KingTechSettings.CurrentCooldownGui then
                    KingTechSettings.CurrentCooldownGui:Destroy()
    ...
    KingTechTab:Slider({
        ...
        Value = {Min = 0.05, Max = 0.8, Default = 0.25},
        Step = 0.01,
        Callback = function(value)
            KingTechSettings.DetectDelay = value
        end
    })
    ...
    KingTechTab:Slider({
        ...
        Step = 0.01,
        Callback = function(value)
            KingTechSettings.StartDelay = value
        end
    })
    ...
    KingTechTab:Slider({
        ...
        Step = 0.01,
        Callback = function(value)
            KingTechSettings.WaitTime = value
        end
    })
    ...
    KingTechTab:Slider({
        ...
        Step = 5,
        Callback = function(value)
            KingTechSettings.VelocityPower = value
        end
    })
    
    KingTechTab:Slider({
        Title = "Cooldown Time (s)",
        Value = {Min = 2, Max = 10, Default = 4},
        Step = 0.5,
        Callback = function(value)
            KingTechSettings.CooldownTime = value
        end
    })
    
    -- ============================================
    -- LOOP DASH TAB (GIỮ NGUYÊN)
    -- ============================================
    ...
    -- INSTANT LETHAL, AUTO KYOTO, LETHAL DASH, LIX TECH, SUPATECH, OREO TECH, TWISTED, KITTY TECH, MISC & VISUAL
    -- ============================================
    -- Instant Lethal Tab
    ...
    InstantLethalTab:Section({ Title = "Instant Lethal Settings", Icon = "settings" })
    InstantLethalTab:Toggle({ Title = "Enable Instant Lethal", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if InstantLethalConfig.Enabled == state then return end
        InstantLethalConfig.Enabled = state
        if state then ConnectInstantLethal() else if InstantLethalConfig.Connection then InstantLethalConfig.Connection:Disconnect(); InstantLethalConfig.Connection = nil end end
    end})
    InstantLethalTab:Slider({ Title = "Smoothness", Value = {Min = 0.1, Max = 1, Default = 0.22}, Step = 0.01, Callback = function(value) InstantLethalConfig.Smoothness = value end })
    
    -- Auto Kyoto Tab
    local KyotoTab = Window:Tab({ Title = "Auto Kyoto", Icon = "activity", IconColor = PINK })
    KyotoTab:Section({ Title = "Auto Kyoto Settings", Icon = "settings" })
    KyotoTab:Toggle({ Title = "Enable Auto Kyoto", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if AutoKyotoConfig.Enabled == state then return end
        AutoKyotoConfig.Enabled = state
        if state then local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid") if hum then SetupKyotoListener(hum) end
        else if AutoKyotoConfig.Connection then AutoKyotoConfig.Connection:Disconnect(); AutoKyotoConfig.Connection = nil end end
    end})
    KyotoTab:Slider({ Title = "Speed", Value = {Min = 5, Max = 50, Default = 22.5}, Step = 0.5, Callback = function(value) AutoKyotoConfig.Speed = value end })
    KyotoTab:Slider({ Title = "Animation Delay", Value = {Min = 0.1, Max = 5, Default = 1.5}, Step = 0.1, Callback = function(value) AutoKyotoConfig.AnimationDelay = value end })
    KyotoTab:Slider({ Title = "Execution Delay", Value = {Min = 0.1, Max = 2, Default = 0.6}, Step = 0.05, Callback = function(value) AutoKyotoConfig.ExecutionDelay = value end })
    
    -- Lethal Dash Tab
    local DashLethalTab = Window:Tab({ Title = "Lethal Dash", Icon = "arrow-right", IconColor = PINK })
    DashLethalTab:Section({ Title = "Lethal Dash Settings", Icon = "settings" })
    DashLethalTab:Toggle({ Title = "Enable Lethal Dash", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if LethalDashConfig.Enabled == state then return end
        LethalDashConfig.Enabled = state
        if state then ConnectLethalDash() else if LethalDashConfig.Connection then LethalDashConfig.Connection:Disconnect(); LethalDashConfig.Connection = nil end; if LethalDashConfig.LerpConnection then LethalDashConfig.LerpConnection:Disconnect(); LethalDashConfig.LerpConnection = nil end; LethalDashConfig.Busy = false; local _, hum = GetCharacterData(); if hum then hum.AutoRotate = true end end
    end})
    DashLethalTab:Slider({ Title = "Wait Time", Value = {Min = 0.5, Max = 3, Default = 1.5}, Step = 0.05, Callback = function(value) LethalDashConfig.WaitTime = value end })
    DashLethalTab:Slider({ Title = "Snap Duration", Value = {Min = 0.1, Max = 1, Default = 0.38}, Step = 0.02, Callback = function(value) LethalDashConfig.SnapDuration = value end })
    DashLethalTab:Slider({ Title = "Lock Distance", Value = {Min = 5, Max = 50, Default = 15}, Step = 1, Callback = function(value) LethalDashConfig.LockDistance = value end })
    
    -- Lix Tech Tab
    local LixTab = Window:Tab({ Title = "Lix Tech", Icon = "zap-2", IconColor = PINK })
    LixTab:Section({ Title = "⚠️ Important Notice", Icon = "alert-triangle" })
    LixTab:Paragraph({ Title = "English", Content = "Turn off Shift Lock before using Lix Tech!" })
    LixTab:Paragraph({ Title = "Tiếng Việt", Content = "Tắt Shiftlock trước khi dùng Lix Tech!" })
    LixTab:Section({ Title = "Lix Tech Settings", Icon = "settings" })
    LixTab:Toggle({ Title = "Enable Lix Tech", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if LixTechConfig.Enabled == state then return end
        LixTechConfig.Enabled = state
        if state then ConnectLixTech() else if LixTechConfig.Connection then LixTechConfig.Connection:Disconnect(); LixTechConfig.Connection = nil end; LixTechConfig.Busy = false; local _, hum = GetCharacterData(); if hum then hum.AutoRotate = true end end
    end})
    LixTab:Slider({ Title = "Delay", Value = {Min = 0.1, Max = 2, Default = 0.3}, Step = 0.05, Callback = function(value) LixTechConfig.Delay = value end })
    
    -- Supa Tech Tab
    local SupaLegitTab = Window:Tab({ Title = "Supa Tech", Icon = "zap", IconColor = PINK })
    SupaLegitTab:Section({ Title = "Supa Tech Settings", Icon = "settings" })
    SupaLegitTab:Toggle({ Title = "Enable Supa Tech", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if SupaLegitConfig.Enabled == state then return end
        SupaLegitConfig.Enabled = state
        if state then ConnectSupaLegit(); if not SupaLegitConfig.CharConnection then SupaLegitConfig.CharConnection = LocalPlayer.CharacterAdded:Connect(function() if SupaLegitConfig.Enabled then task.wait(0.5); ConnectSupaLegit() end end) end
        else if SupaLegitConfig.Connection then SupaLegitConfig.Connection:Disconnect(); SupaLegitConfig.Connection = nil end; if SupaLegitConfig.CharConnection then SupaLegitConfig.CharConnection:Disconnect(); SupaLegitConfig.CharConnection = nil end; SupaLegitConfig.Busy = false; SupaLegitConfig.InCooldown = false; local _, hum = GetCharacterData(); if hum then hum.AutoRotate = true end end
    end})
    SupaLegitTab:Slider({ Title = "Dash Duration", Value = {Min = 0.05, Max = 0.5, Default = 0.15}, Step = 0.01, Callback = function(value) SupaLegitConfig.DashDuration = value end })
    SupaLegitTab:Slider({ Title = "Follow Offset", Value = {Min = 0.5, Max = 6, Default = 2.5}, Step = 0.1, Callback = function(value) SupaLegitConfig.FollowOffset = value end })
    SupaLegitTab:Slider({ Title = "Angle Tilt (Degrees)", Value = {Min = 10, Max = 90, Default = 55}, Step = 1, Callback = function(value) SupaLegitConfig.AngleTilt = value end })
    SupaLegitTab:Slider({ Title = "Stick Range", Value = {Min = 5, Max = 35, Default = 18}, Step = 1, Callback = function(value) SupaLegitConfig.StickRange = value end })
    SupaLegitTab:Slider({ Title = "Cooldown Time", Value = {Min = 1, Max = 10, Default = 4}, Step = 0.5, Callback = function(value) SupaLegitConfig.CooldownTime = value end })
    
    -- Oreo Tech Tab
    local OreoTechTab = Window:Tab({ Title = "Oreo Tech", Icon = "hexagon", IconColor = PINK })
    OreoTechTab:Section({ Title = "Oreo Tech Settings", Icon = "settings" })
    OreoTechTab:Toggle({ Title = "Enable Oreo Tech", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if OreoTechConfig.Enabled == state then return end
        OreoTechConfig.Enabled = state
        if state then ConnectOreoTech(); if not OreoTechConfig.CharConnection then OreoTechConfig.CharConnection = LocalPlayer.CharacterAdded:Connect(function() if OreoTechConfig.Enabled then task.wait(0.5); ConnectOreoTech() end end) end
        else if OreoTechConfig.Connection then OreoTechConfig.Connection:Disconnect(); OreoTechConfig.Connection = nil end; if OreoTechConfig.CharConnection then OreoTechConfig.CharConnection:Disconnect(); OreoTechConfig.CharConnection = nil end; OreoTechConfig.CanTrigger = true; local _, hum = GetCharacterData(); if hum then hum.AutoRotate = true end end
    end})
    OreoTechTab:Slider({ Title = "Cooldown Time", Value = {Min = 1, Max = 10, Default = 5}, Step = 0.5, Callback = function(value) OreoTechConfig.CooldownTime = value end })
    
    -- Twisted Tab
    local TwistedTab = Window:Tab({ Title = "Twisted", Icon = "repeat", IconColor = PINK })
    TwistedTab:Section({ Title = "Twisted Revamp Settings", Icon = "settings" })
    TwistedTab:Toggle({ Title = "Enable Twisted", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if TwistedConfig.Enabled == state then return end
        TwistedConfig.Enabled = state
        if state then ConnectTwisted() else if TwistedConfig.Connection then TwistedConfig.Connection:Disconnect(); TwistedConfig.Connection = nil end; if TwistedConfig.AutoRotateConnection then TwistedConfig.AutoRotateConnection:Disconnect(); TwistedConfig.AutoRotateConnection = nil end; TwistedConfig.Busy = false; local _, hum = GetCharacterData(); if hum then hum.AutoRotate = true end end
    end})
    
    -- Kitty Tech Tab
    local KittyTechTab = Window:Tab({ Title = "Kitty Tech", Icon = "cat", IconColor = PINK })
    KittyTechTab:Section({ Title = "Kitty Tech Settings", Icon = "settings" })
    KittyTechTab:Toggle({ Title = "Enable Kitty Tech", Value = false, Icon = "power", Color = PINK, Callback = function(state)
        if KittyTechConfig.Enabled == state then return end
        KittyTechConfig.Enabled = state
        if state then ConnectKittyTech(); if not KittyTechConfig.CharConnection then KittyTechConfig.CharConnection = LocalPlayer.CharacterAdded:Connect(function() if KittyTechConfig.Enabled then task.wait(0.5); ConnectKittyTech() end end) end
        else if KittyTechConfig.Connection then KittyTechConfig.Connection:Disconnect(); KittyTechConfig.Connection = nil end; if KittyTechConfig.CharConnection then KittyTechConfig.CharConnection:Disconnect(); KittyTechConfig.CharConnection = nil end; KittyTechConfig.Busy = false; local _, hum = GetCharacterData(); if hum then hum.AutoRotate = true; hum.WalkSpeed = 16; hum.JumpPower = 50; hum.PlatformStand = false end end
    end})
    KittyTechTab:Slider({ Title = "Dash Delay", Value = {Min = 0.001, Max = 0.5, Default = 0.01}, Step = 0.001, Callback = function(value) KittyTechConfig.DashDelay = value end })
    KittyTechTab:Slider({ Title = "Post Dash Wait", Value = {Min = 0.01, Max = 0.5, Default = 0.078}, Step = 0.001, Callback = function(value) KittyTechConfig.PostDashWait = value end })
    KittyTechTab:Slider({ Title = "Cooldown Time", Value = {Min = 0.5, Max = 10, Default = 4.8}, Step = 0.1, Callback = function(value) KittyTechConfig.CooldownTime = value end })
    KittyTechTab:Slider({ Title = "Orbit Radius", Value = {Min = 0.5, Max = 10, Default = 3}, Step = 0.5, Callback = function(value) KittyTechConfig.OrbitRadius = value end })
    KittyTechTab:Slider({ Title = "Orbit Height", Value = {Min = 0, Max = 5, Default = 1}, Step = 0.5, Callback = function(value) KittyTechConfig.OrbitHeight = value end })
    KittyTechTab:Slider({ Title = "Spin Duration", Value = {Min = 0.1, Max = 2, Default = 0.6}, Step = 0.05, Callback = function(value) KittyTechConfig.SpinDuration = value end })
    KittyTechTab:Slider({ Title = "Target Distance", Value = {Min = 5, Max = 30, Default = 12}, Step = 1, Callback = function(value) KittyTechConfig.TargetDistance = value end })
    KittyTechTab:Slider({ Title = "Attachment Duration", Value = {Min = 0.1, Max = 1.5, Default = 0.5}, Step = 0.05, Callback = function(value) KittyTechConfig.AttachmentDuration = value end })
    KittyTechTab:Slider({ Title = "Ping Setting", Value = {Min = 20, Max = 200, Default = 60}, Step = 5, Callback = function(value) KittyTechConfig.PingSetting = value end })
    
    -- Misc & Visual Tab (giữ nguyên các emote và no cooldown dash)
    local MiscVisualTab = Window:Tab({ Title = "Misc & Visual", Icon = "settings", IconColor = PINK })
    local GeneralSection = MiscVisualTab:Section({ Title = "General Settings", Icon = "settings" })
    GeneralSection:Button({ Title = "Support", Description = "Support Discord", Icon = "heart", Color = PINK, Callback = function()
        local success = CopyToClipboard("https://discord.gg/W8vpy5zH5")
        if success then WindUI:Notify({ Title = "Copied!", Content = "Discord support link copied!", Icon = "check-circle", Duration = 3 }) else WindUI:Notify({ Title = "Error!", Content = "Cannot copy link", Icon = "alert-circle", Duration = 3 }) end
    end})
    GeneralSection:Toggle({ Title = "No Cooldown Dash", Description = "Remove dash cooldown (experimental)", Value = false, Icon = "zap", Color = PINK, Callback = function(state)
        NoCooldownDashConfig.Enabled = state
        SetupNoCooldownDash()
        if state then WindUI:Notify({ Title = "No Cooldown Dash", Content = "Enabled! (May not work on all servers)", Icon = "zap", Duration = 3 }) else WindUI:Notify({ Title = "No Cooldown Dash", Content = "Disabled!", Icon = "power", Duration = 2 }) end
    end})
    
    local EmoteSection = MiscVisualTab:Section({ Title = "Emote Limited", Icon = "smile" })
    EmoteSection:Button({ Title = "Final Stand", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayFinalStand })
    EmoteSection:Button({ Title = "Inner Rage", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayInnerRage })
    EmoteSection:Button({ Title = "Shadow Eruption", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayShadowEruption })
    EmoteSection:Button({ Title = "Divine Form", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayDivineForm })
    EmoteSection:Button({ Title = "The Strongest", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayTheStrongest })
    EmoteSection:Button({ Title = "Boundless Rage", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayBoundlessRage })
    EmoteSection:Button({ Title = "The Fallen", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayTheFallen })
    EmoteSection:Button({ Title = "True Aura", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayTrueAura })
    EmoteSection:Button({ Title = "Eternal Seal (New)", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayEternalSeal })
    EmoteSection:Button({ Title = "World Cutting Slash", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayWorldCuttingSlash })
    EmoteSection:Button({ Title = "My Brother", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayMyBrother })
    EmoteSection:Button({ Title = "Final Spark", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayFinalSpark })
    EmoteSection:Button({ Title = "Last Will", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayLastWill })
    EmoteSection:Button({ Title = "The Fallen Finisher", Icon = "mouse-pointer-click", Color = PINK, Callback = PlayTheFallenFinisher })
    ...
    local function PlayMusic(soundId, volume)
        ...
        musicSound.Volume = volume or 0.5
    ...
    local function SetVolume(vol)
        if musicSound then
            musicSound.Volume = math.clamp(vol, 0, 1)
        end
    end
    
    MusicTab:Paragraph({
        Title = "Now Playing",
        Content = "Call Of Silence (ID: 123878926068911)",
    })
    
    MusicTab:Button({
        Title = "▶ Play Call Of Silence",
        Icon = "play",
        Color = PINK,
        Callback = function()
            PlayMusic("123878926068911", 0.5)
            WindUI:Notify({ Title = "Music", Content = "Playing Call Of Silence", Icon = "music", Duration = 2 })
        end
    })
    ...
    MusicTab:Button({
        ...
        Callback = function()
            StopMusic()
            WindUI:Notify({ Title = "Music", Content = "Music stopped", Icon = "square", Duration = 2 })
        end
    })
    ...
    MusicTab:Button({
        ...
        Callback = function()
            if musicSound then
                local newVol = math.min(musicSound.Volume + 0.1, 1)
                SetVolume(newVol)
                WindUI:Notify({ Title = "Volume", Content = "Volume: " .. math.floor(newVol * 100) .. "%", Icon = "volume-2", Duration = 1.5 })
            else
                WindUI:Notify({ Title = "No music", Content = "Play a song first!", Icon = "alert-circle", Duration = 2 })
            end
        end
    })
    ...
    MusicTab:Button({
        ...
        Callback = function()
            if musicSound then
                local newVol = math.max(musicSound.Volume - 0.1, 0)
                SetVolume(newVol)
                WindUI:Notify({ Title = "Volume", Content = "Volume: " .. math.floor(newVol * 100) .. "%", Icon = "volume-1", Duration = 1.5 })
            else
                WindUI:Notify({ Title = "No music", Content = "Play a song first!", Icon = "alert-circle", Duration = 2 })
            end
        end
    ...
    MusicTab:Slider({
        ...
        Callback = function(value)
            SetVolume(value / 100)
            if musicSound then
                WindUI:Notify({ Title = "Volume", Content = "Volume set to " .. value .. "%", Icon = "sliders", Duration = 1 })
            end
        end
    })
...
-- ============================================
-- CHARACTER SPAWN HANDLING (GIỮ NGUYÊN)
-- ============================================

...
local function PlayMusic(trackId, volume)
    -- Stop current music if playing
    if MusicConfig.CurrentSound then
        pcall(function()
            MusicConfig.CurrentSound:Stop()
            MusicConfig.CurrentSound:Destroy()
        end)
        MusicConfig.CurrentSound = nil
    end
    
    local sound = Instance.new("Sound")
    sound.SoundId = "rbxassetid://" .. trackId
    sound.Volume = volume or MusicConfig.Volume
    sound.Looped = true
    sound.Parent = workspace
    sound:Play()
    
    MusicConfig.CurrentSound = sound
    MusicConfig.CurrentTrack = trackId
    
    -- Cleanup when music ends
    sound.Ended:Connect(function()
        if MusicConfig.CurrentSound == sound then
            MusicConfig.CurrentSound = nil
            MusicConfig.CurrentTrack = nil
        end
        sound:Destroy()
    end)
...
local function SetMusicVolume(volume)
    MusicConfig.Volume = math.clamp(volume, 0, 1)
    if MusicConfig.CurrentSound then
        MusicConfig.CurrentSound.Volume = MusicConfig.Volume
    end
end
...
local K1ngTechConfig = {
    ...
    LockDistance = 14,
    CooldownTime = 4,
    UppercutAnims = {"10503381238", "13379003796"},
    DetectAnims = {"10479335397", "13380255751"},
    Connection = nil,
    CharConnection = nil,
    OnCooldown = false,
    Busy = false,
...
local function K1ngTechGetClosestEnemy(maxDist)
...
local function K1ngTechAutoPressQ()
    local char = LocalPlayer.Character
    if char then
        local comm = char:FindFirstChild("Communicate")
        if comm then
...
local function K1ngTechForceJump()
    local char = LocalPlayer.Character if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart") if not root then return end
    root.AssemblyLinearVelocity = Vector3.new(root.AssemblyLinearVelocity.X, K1ngTechConfig.VelocityPower, root.AssemblyLinearVelocity.Z)
end

local function K1ngTechCustomFlip()
    ...
    if hum and root then
        ...
        local camPos = flippedCFrame.Position - flippedCFrame.LookVector * (Camera.CFrame.Position - flippedCFrame.Position).Magnitude + Vector3.new(0, 2)
        Camera.CFrame = CFrame.new(camPos, flippedCFrame.Position)
    end
end

local function K1ngTechStartCooldown()
    K1ngTechConfig.OnCooldown = true
    task.delay(K1ngTechConfig.CooldownTime, function()
        K1ngTechConfig.OnCooldown = false
    end)
end

local function K1ngTechExecute(triggerTime)
    if K1ngTechConfig.Busy or not K1ngTechConfig.Enabled or K1ngTechConfig.OnCooldown then return end
    if tick() - triggerTime > 0.4 then return end
    
    local enemy = K1ngTechGetClosestEnemy(K1ngTechConfig.LockDistance)
    ...
    task.delay(K1ngTechConfig.StartDelay, function()
        ...
        K1ngTechAutoPressQ()
        K1ngTechForceJump()
        task.wait(K1ngTechConfig.WaitDelay)
        K1ngTechCustomFlip()
        task.wait(0.08)
        K1ngTechCustomFlip()
        ...
        if hrp then
            TweenService:Create(hrp, TweenInfo.new(0.12, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
...
local function K1ngTechOnAnimation(animTrack)
    if not K1ngTechConfig.Enabled or K1ngTechConfig.OnCooldown then return end
    ...
    -- Check for uppercut animations
    local isUppercut = false
    for _, id in ipairs(K1ngTechConfig.UppercutAnims) do
    ...
    -- Check for detect animations (for cooldown)
    for _, id in ipairs(K1ngTechConfig.DetectAnims) do
        if animId:find(id) and not K1ngTechConfig.OnCooldown then
            K1ngTechStartCooldown()
            break
        end
    end
    
    if isUppercut and not K1ngTechConfig.OnCooldown then
        local triggerTime = tick()
        task.delay(K1ngTechConfig.DetectDelay, function()
            K1ngTechExecute(triggerTime)
        end)
    end
end
...
local function ConnectK1ngTech()
    ...
    K1ngTechConfig.Connection = hum.AnimationPlayed:Connect(K1ngTechOnAnimation)
end
...
if not WindUI then
    ...
    K1ngTechTab:Section({
        Title = "K1ng Tech Settings",
        Icon = "settings",
    })
    
    K1ngTechTab:Paragraph({
        Title = "Thông Tin",
        Content = "K1ng Tech - Auto uppercut dash combo with detection",
    })
    ...
    K1ngTechTab:Toggle({
        ...
        Color = PINK,
        Callback = function(state)
            if K1ngTechConfig.Enabled == state then return end
            K1ngTechConfig.Enabled = state
            if state then
                ConnectK1ngTech()
                if not K1ngTechConfig.CharConnection then
                    K1ngTechConfig.CharConnection = LocalPlayer.CharacterAdded:Connect(function()
    ...
    K1ngTechTab:Slider({
        ...
        Step = 0.01,
        Callback = function(value)
            K1ngTechConfig.WaitDelay = value
        end
    })
    
    K1ngTechTab:Slider({
        Title = "Velocity Power",
        Value = {Min = 20, Max = 100, Default = 60},
        Step = 5,
        Callback = function(value)
            K1ngTechConfig.VelocityPower = value
        end
    })
    ...
    K1ngTechTab:Slider({
        ...
        Step = 1,
        Callback = function(value)
            K1ngTechConfig.LockDistance = value
        end
    })
    ...
    MusicTab:Paragraph({
        Title = "Thông Tin",
        Content = "Nghe nhạc trong game! Music sẽ chạy nền và loop liên tục.",
    })
    
    -- Chọn nhạc dropdown
    local selectedTrack = "Call Of Silence"
    
    MusicTab:Dropdown({
        Title = "Chọn nhạc",
        Values = {"Call Of Silence"},
        Default = 1,
        Icon = "list",
        Callback = function(value)
            selectedTrack = value
    ...
    MusicTab:Button({
        ...
        Callback = function()
            ...
            if trackId then
                PlayMusic(trackId, MusicConfig.Volume)
                WindUI:Notify({
                    Title = "Music",
                    Content = "Đang phát: " .. selectedTrack,
                    Icon = "music",
                    Duration = 3,
                })
            else
                WindUI:Notify({
                    Title = "Error",
                    Content = "Không tìm thấy track nhạc!",
                    Icon = "alert-circle",
                    Duration = 3,
                })
            end
